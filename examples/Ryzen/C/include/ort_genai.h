...
struct OgaModel : OgaAbstract {
  ...
  // cfg_overlay : the items in genai_config.json that you want to overwrite with new value. for example:
  // std::string str= "{ \"model\": { \"decoder\": { \"session_options\": { \"hybrid_opt_init_prompt_size\": \"1024\",  \"hybrid_opt_npu_read_ahead\": \"2\",  \"rmm_memory_limit_mb\": 1024,  \"hybrid_opt_gpu_jit\": \"2\" } } } }  ";
  static std::unique_ptr<OgaModel> Create(const char* config_path, const char* cfg_overlay) {
    OgaModel* p;
    OgaCheckResult(OgaCreateModelWithOverlayConfig(config_path, cfg_overlay, &p));
    return std::unique_ptr<OgaModel>(p);
  }
  ...
  static std::unique_ptr<OgaModel> Create(const char* config_path, const void* model_data, size_t model_data_length, const void* external_data, size_t external_data_length, const void* header_data, size_t header_data_length, const char* cfg_overlay=nullptr) {
    OgaModel* p;
    OgaCheckResult(OgaCreateModelFromMemory(config_path, model_data, model_data_length, external_data, external_data_length, header_data, header_data_length, cfg_overlay, &p));
    return std::unique_ptr<OgaModel>(p);
  }
...
struct OgaGenerator : OgaAbstract {
  ...
  void SetActiveRyzenAdapter(const char* lora_bin_prefix) {
    OgaCheckResult(OgaSetActiveRyzenAdapter(this, lora_bin_prefix));
  }
