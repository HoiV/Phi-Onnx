 ...
 * \brief Creates a model from the given configuration directory, runtime settings and device type.
 * \param[in] config_path The path to the model configuration directory. The path is expected to be encoded in UTF-8.
 * \param[in] cfg_overlay  the config item want to overlay;
 * \param[out] out The created model.
 * \return OgaResult containing the error message if the model creation failed.
 */
OGA_EXPORT OgaResult* OGA_API_CALL OgaCreateModelWithOverlayConfig(const char* config_path, const char* cfg_overlay, OgaModel** out);
 ...
 * \return OgaResult containing the error message if the model creation failed.
 */
OGA_EXPORT OgaResult* OGA_API_CALL OgaCreateModelFromMemory(const char* config_path, const void* model_data, size_t model_data_length, const void* external_data, size_t external_data_length, const void* header_data, size_t header_data_length, const char* cfg_overlay, OgaModel** out);
...
/**
 * \brief Sets the ryzen adapter with the given ryzen adapter name as active for the given OgaGenerator object.
 * \param[in] generator The OgaGenerator object to set the active adapter.
 * \param[in] lora_bin_prefix The name prefix of the LoRA adapter bin file.
 */
OGA_EXPORT OgaResult* OGA_API_CALL OgaSetActiveRyzenAdapter(OgaGenerator* generator,
                                                        const char* lora_bin_prefix);
